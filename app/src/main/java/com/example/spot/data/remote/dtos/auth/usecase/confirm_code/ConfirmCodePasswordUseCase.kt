package com.example.spot.data.remote.dtos.auth.usecase.confirm_code
...
class ConfirmCodePasswordUseCase(
    private val repository: AuthRepository
) {

    suspend operator fun invoke(email: String, code: String) : Result {

        try {
            val request = ConfirmCodePasswordRequest(email, code)
            repository.confirmCodePassword(request)
